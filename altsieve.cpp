...
int main(int argc, char* argv[]) {
	...
	// If N is a "reasonable" number, i.e. below 100 million,
	if (N2 < 1e8) 
	{
	...
	// If N is really big, do this
	// Warning: this has no safeguards. It will try to allocate however much RAM is necessary 
	// for the upper bound specified in chunks of 10 million long ints, or 80MB
	else 
	{
		const int64_t array_size = 10000000;
		const int64_t num_arrays = ((N2/2) / array_size) + int((N2/2) % array_size > 0);
